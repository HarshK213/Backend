...
const publishAVideo = asyncHandler(async (req, res) => {
  ...
  const videoLink = await cloudinaryFileUpload(videoLocalPath, {
    resource_type: "video",
  });
  if (!videoLink) {
  ...
  if (!videoCreated) {
    throw new ApiError(
      400,
      "Something went wrong while registering video file"
    );
  }
...
const updateVideo = asyncHandler(async (req, res) => {
  ...
  const oldPulicId = oldThumbnailLink.split('/').pop().split('.')[0];;
...
const deleteVideo = asyncHandler(async (req, res) => {
  ...
  const oldVideoLink = video.videoFile;
  const oldVideoId = oldVideoLink.split('/').pop().split('.')[0];;
  await cloudinaryFileDelete(oldVideoId, 'video');
